...
class MazeEnvExtended(MazeEnv):
    ...
    def step(self, action):
        ...
        if (prev_obs is not None
                ...
                and self.trace):
            self.trace_count[int(prev_obs[0]), int(prev_obs[1])] = self.trace_count[int(prev_obs[0]), int(prev_obs[1])] +1
            self._draw_trace(prev_obs)
        
        return obs, reward, done, info
    
    def _draw_trace(self, position):
        x = int(position[0] * self.maze_view.CELL_W + self.maze_view.CELL_W * 0.5 + 0.5)
        y = int(position[1] * self.maze_view.CELL_H + self.maze_view.CELL_H * 0.5 + 0.5)
        r = int(min(self.maze_view.CELL_W, self.maze_view.CELL_H)/5 + 0.5)

        opacity = min([self.trace_count[int(position[0]), int(position[1])]*50, 255])

        pygame.draw.circle(self.maze_view.maze_layer, (0, 150, 0, opacity), (x, y), r)
